...
Probably a package manager is better to create easier reproducible envirenmonts. Or docker?

** Conda
A systemwide packagemanager: it can install different versions of python, python packages and other dependencies (e.g., libraries). 
This unlike pip, which can only install python packages. It is a cross-platform package manager, supporting linux macOS and Windows, so it is great for code portability. 
It is also an environment manager, meaning you can have different set-ups, i.e., versions of python and installed python libraries, for different projects. 

*** Install
I prefer the miniconda distribution, which lets you choose all the packages you want installed, over anaconda, which just installs a ton of packages by default.  
