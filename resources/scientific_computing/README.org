...
** Easier logging in
It is cumbersome to have to have to enter your username and password each time.

To avoid having to enter the password each time, we will generate an ssh key. On your local machine run:
#+begin_src bash
  ssh-keygen
#+end_src
This will generate a private key (under ~/.ssh/id_rsa, by default) and
a public key (under ~/.ssh/id_rsa.pub, by default).

To copy the accepted private key to /mn1/ run:
#+begin_src bash
ssh-copy-id  -i .ssh/id_rsa mn1
#+end_src

From now on, whenever we ssh to the remote machine, the instead of being prompted for the password, we automatically offer the public id_rsa.pub file, which well be checked by the host to match (after hashing) to the copied id_rsa file. 

Now, on the local machine, we create an alias /mn1/ so that each time
we ssh to the /mn1/ node, we do so using the user /bscuser/ and use
our generated key instead of a password. In the file '.ssh/config',
