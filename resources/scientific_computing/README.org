...
I prefer the Micromamba distribution over the default Mamba distribution, as it is a lot smaller and only installs the bare minimum needed packages to get up and running.
...
Like any package manager, Mamba has a list of sources for packages it can install, called /channels/. To see the available channels run: 
